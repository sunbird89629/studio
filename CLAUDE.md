...
- `flex_tabs` `Tabs.activate()` ordering: calls `notifyListeners()` BEFORE setting `document.activeTab.value = tab` — any ChangeNotifier listener that calls `_computeState()` during the notification reads a stale null `activeTab`; `_TabsDocumentBridge` must also listen to `document.activeTab` (a separate ValueNotifier) to stay current
- `flex_tabs` tab content rendering: only the active tab's content widget is in the widget tree at any time — switching tabs UNMOUNTS the old `TerminalTabView` and MOUNTS a new one, so `initState` fires on every tab switch (use `addPostFrameCallback` in `initState` to request terminal focus)
- macOS `PlatformMenuBar` + `HardwareKeyboard` double-firing: shortcuts in `PlatformMenuBar.onSelected` (native menu) also reach Flutter's `HardwareKeyboard` handlers — do NOT add shortcuts to `_passthroughIntents` that are already in `PlatformMenuBar`, or non-idempotent actions (e.g. `newTab`) will execute twice; `_initTabs()` must call `setRoot(root)` BEFORE `openTerminal()` so `tab.activate()` can access the document
